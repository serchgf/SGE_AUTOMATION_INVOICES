...
def pytest_configure(config):
    ...
    config.option.htmlpath = f"{report_directory}SGE_Create_invoice_Test_Report.html"
...
#     report.title = "SGE Create Invoice Test Report"

# estas funciones me permiten editar el titulo y adjuntar una imagen
# def pytest_html_report_title(report):
#     test_files = set()
#     str_name=''
...
#         #usando la tupla
#         str_name = f"SGE Invoice Report: {''.join(str(test_files))}"
#         #report.title = f"SGE Invoice Report: {''.join(tmp[0])}"
#     report.title = str_name.replace(',', ' ').replace('.py', ''). replace('_', ' ')
